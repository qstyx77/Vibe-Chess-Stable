...
import { LootWinningsWindow } from '@/components/evolving-chess/LootWinningsWindow';
...
import type { BoardState, PlayerColor, AlgebraicSquare, Piece, Move, GameStatus, PieceType, Effect, InventoryItem, InventoryItemType, AIGameState, AIBoardState, AISquareState, SquareState, ItemType, ChatMessage, MessageCategory, RookResurrectionResult, AIMove as AIMoveType } from '@/types';
...
function adaptBoardForAI(
  ...
  pColor: PlayerColor, 
  ks: { white: number; black: number }, 
  caps: { white: Piece[]; black: Piece[] }, 
  moveC: number, 
  fb: boolean, 
  fbP: PlayerColor | null, 
  ep: AlgebraicSquare | null, 
  lmT?: PieceType | null, 
  sC?: number, 
  nsT?: number, 
  lmH?: InventoryItemType | null, 
  lmL?: number | null, 
  oppC?: boolean, 
  posH?: string[],
  nrC?: number
  ...
  return { 
    ...
    killStreaks: { ...ks }, 
    capturedPieces: { white: Array.isArray(caps?.white) ? caps.white.map(p => ({ ...p })) : [], black: Array.isArray(caps?.black) ? caps.black.map(p => ({ ...p })) : [] }, 
    gameOver: false, 
    winner: undefined, 
    extraTurn: false, 
    gameMoveCounter: moveC, 
    firstBloodAchieved: fb, 
    playerWhoGotFirstBlood: fbP, 
    enPassantTargetSquare: ep, 
    shroomSpawnCounter: sC, 
    nextShroomSpawnTurn: nsT, 
    necroResurrectionCounter: nrC, 
    lastMovedPieceType: lmT, 
    lastMovedPieceHeldItem: lmH, 
    lastMovedPieceLevel: lmL, 
    didOpponentCaptureLastTurn: oppC, 
    positionHistory: posH ? [...posH] : [] 
  };
}
...
export default function DungeonPage() {
  ...
  const [lootFound, setLootFound] = useState<InventoryItemType[]>([]);
  const [pendingProgression, setPendingProgression] = useState<{ survivors: Piece[], graveyard: any } | null>(null);
  ...
  const handlePieceHover = useCallback((p: Piece | null) => {
    ...
    if (p && p.color !== currentPlayer && !isSpec) {
  ...
  }, [board, currentPlayer, enPassantTargetSquare, lastMovedPieceType, lastMovedPieceHeldItem, lastMovedPieceLevel, isSpec, clickedEnemySquare]);

  const hasStartedFloor = useMemo(() => board.flat().some(sq => sq.piece?.color === 'white' && sq.piece.hasMoved), [board]);

  const proceedToNextFloor = useCallback((nextL: number, survivors: Piece[], graveyard: any, newInv?: InventoryItem[]) => {
    ...
    const finalInv = newInv || inventory;
    ...
    saveDungeonState(nextL, generateDungeonFloor(nextL, survivors), 'white', { white: 0, black: 0 }, { white: graveyard.white, black: [] }, 0, 5, null, 0, finalInv);
    audioManager.playLevelUp(); addLog(`Descending to Floor ${nextL}...`);
  }, [inventory, saveDungeonState, addLog]);

  const handleClaimLoot = useCallback(() => {
    if (!pendingProgression) return;
    const updatedInv = [...inventory];
    lootFound.forEach(type => {
        const existing = updatedInv.find(i => i.type === type);
        if (existing) existing.count++;
        else updatedInv.push({ type, count: 1 });
    });
    setInventory(updatedInv);
    setIsLootWindowOpen(false);
    const survivors = pendingProgression.survivors;
    const graveyard = pendingProgression.graveyard;
    setPendingProgression(null);
    setLootFound([]);
    proceedToNextFloor(level + 1, survivors, graveyard, updatedInv);
  }, [pendingProgression, lootFound, inventory, level, proceedToNextFloor]);
  ...
  const advanceLevel = useCallback((survivors: Piece[], graveyard: any) => {
    if (level % 10 === 0) {
        ...
        allItems.forEach(type => {
            ...
            if (Math.random() < chance) drops.push(type);
        });
        setLootFound(drops);
        setPendingProgression({ survivors, graveyard });
        setIsLootWindowOpen(true);
    } else {
        proceedToNextFloor(level + 1, survivors, graveyard);
    }
  }, [level, proceedToNextFloor]);
  ...
  const processMoveEnd = useCallback((boardAfter: BoardState, nG: any, curKs: any, turnP: PlayerColor, extra: boolean, nEp: AlgebraicSquare | null = null, wasCap: boolean = false, movedT?: PieceType | null) => {
    ...
    // Stalemate for Black (AI) no longer triggers immediate advancement here.
    // Instead, nxtP will be 'black' and performAiMove will handle the turn counter.
    if (cleared || (isBoss && mated)) {
        const sur = nB.flat().filter(sq => sq.piece && sq.piece.color === 'white').map(sq => sq.piece!);
        addLog(cleared ? "ALL FOES VANQUISHED!" : "BOSS ENTITY DEFEATED!");
    ...
    if (inC) { addLog("Check!"); audioManager.playCheck(); }
  ...
  const triggerSpecialsChain = useCallback((bCh: BoardState, cG: { white: Piece[], black: Piece[] }, cKs: { white: number, black: number }, oldS: number, newS: number, isEx: boolean, nEp: AlgebraicSquare | null, actP: PlayerColor = 'white', compM: string[] = [], capId: string | null = null, wasCap: boolean = false, movedT?: PieceType | null) => {
        ...
        if (hasD) {
            if (isAI) {
                ...
                if (aiDSq) {
                    ...
                    if (cands.length > 0) {
                        const best = cands[0]; const tSq = nxtB[best.r][best.c]; const tP = tSq.piece; const targetItem = tSq.item;
                        if (targetItem?.type === 'shroom') { nxtB[best.r][best.c].piece = { ...dP, hasMoved: true, level: (dP.level || 1) + 1 }; } else { nxtB[best.r][best.c].piece = { ...dP, hasMoved: true }; }
                        nxtB[best.r][best.c].item = null; nxtB[r][c].piece = tP ? { ...tP, hasMoved: true, isShielded: false } : null; nxtB[r][c].item = targetItem?.type === 'shroom' ? null : targetItem;
                        addLog(`${getPlayerDisplayName(actP)} Dancer performed a free ${tP ? 'swap' : (targetItem ? 'anvil swap' : 'move')}!`);
                    }
                }
    ...
    if (!sil && ((newS >= 5 && oldS < 5 && snipers.length > 0) || (newS >= 3 && oldS < 3 && hasCB)) && !compM.includes('snipe')) {
        ...
        if (vics.length > 0) {
            if (isAI) {
                const nxtB = bCh.map(r => r.map(s => ({...s, piece: s.piece ? {...s.piece} : null, item: s.item ? {...s.item} : null})));
                const vSrt = vics.sort((a,b) => (VAL_MAP[b.piece!.type]||0) - (VAL_MAP[a.piece!.type]||0));
  ...
  const handleSquareClick = useCallback((alg: AlgebraicSquare) => {
    ...
    if (isAwaitingDanceTarget) {
        ...
        if (isAdj && (piece || (sq?.item?.type === 'anvil' && dP?.heldItem === 'dancers_ribbon') || (!sq?.item && row === fr - 1))) {
            ...
            const activeD = nxtB[fr][fc].piece!; const tP = nxtB[row][col].piece; const targetItem = nxtB[row][col].item;
            if (targetItem?.type === 'shroom') { activeD.level = Math.min(activeD.type === 'queen' ? 7 : 99, (activeD.level || 1) + 1); nxtB[row][col].item = null; }
            nxtB[row][col].piece = activeD; nxtB[fr][fc].piece = tP ? { ...tP, hasMoved: true } : null; nxtB[fr][fc].item = targetItem?.type === 'shroom' ? null : targetItem;
    ...
    if (selectedSquare) {
       ...
       if (moves.includes(alg)) {
          ...
          const totalCaptures = (res.capturedPiece ? 1 : 0) + (res.pieceCapturedByAnvil ? 1 : 0) + (res.selfDestructCaptures?.length || 0);
          const captureGainExp = res.capturedPiece ? (DUNGEON_EXP_MAP[res.capturedPiece.type] || 1) : 0;
          if (captureGainExp > 0) addEffect('level-change', alg, 'white', captureGainExp);
          ...
          if ((oT === 'rook' || oT === 'palace') && totalCaptures > 0) {
          ...
          setTimeout(() => { 
            ...
            const oS = killStreaks['white'], nS = (totalCaptures > 0) ? oS + totalCaptures : 0, isExtraTurn = res.extraTurn || (oS < 6 && nS >= 6);
            ...
            if (q.length > 0) { setPromotionQueue(q); setIsPromotingPawn(true); setPromotionSquare(q[0].square); setPromotionTargetLevel(q[0].targetLevel); setSpecialActionContext({ boardForNextStep: nextBoardState, playerWhoseTurnCompleted: 'white', isExtraTurn: isExtraTurn, newEnPassantTarget: res.enPassantTargetSet, oldStreak: oS, newStreak: nS, currentGraveyard: nxtG, currentKs: cKs, capturingPieceId: nextBoardState[row][col].piece?.id || null }); } 
            else { processPawnSacrificeCheck(nextBoardState, nxtG, cKs, 'white', {from: selectedSquare, to: alg, type: mType}, oL, oT, isExtraTurn, res.enPassantTargetSet, oS, nS, nextBoardState[row][col].piece?.id || null, totalCaptures > 0, oT); }
          }, 800);
          return;
       }
    }
    if (piece && piece.color === currentPlayer) { 
        setClickedEnemySquare(null); setEnemySelectedSquare(null); setEnemyPossibleMoves([]);
        setSelectedSquare(alg); setPossibleMoves(getPossibleMoves(board, alg, enPassantTargetSquare, lastMovedPieceType, lastMovedPieceHeldItem, null, lastMovedPieceLevel)); 
    } else if (piece && piece.color !== currentPlayer && !isSpec) {
        setSelectedSquare(null); setPossibleMoves([]);
        const moves = getPossibleMoves(board, alg, enPassantTargetSquare, lastMovedPieceType, lastMovedPieceHeldItem, null, lastMovedPieceLevel);
        setClickedEnemySquare(alg); setEnemySelectedSquare(alg); setEnemyPossibleMoves(moves);
    } else { 
        setSelectedSquare(null); setPossibleMoves([]); 
        setClickedEnemySquare(null); setEnemySelectedSquare(null); setEnemyPossibleMoves([]);
    }
  }, [board, currentPlayer, selectedSquare, enPassantTargetSquare, lastMovedPieceType, lastMovedPieceHeldItem, lastMovedPieceLevel, capturedPieces, killStreaks, isInventoryOpen, selectedInventoryItemType, handlePieceHover, triggerSpecialsChain, boardForPostSacrifice, specialActionContext, isAwaitingPawnSacrifice, isAwaitingCommanderPromotion, isAwaitingAnvilDrop, isAwaitingAnvilScrollTarget, playerToDropAnvil, isAwaitingHolyShield, isAwaitingArcherSnipe, dancerToDance, isAwaitingDanceTarget, processPawnSacrificeCheck, didCaptureLastTurn, addEffect, promotionQueue, promotionTargetLevel, isAwaitingGrappleThrow, grappledPieceSubject, grappledItemSubject, isSelectingMycoSpell, isAwaitingWindScrollTarget, isAwaitingEarthquakeScrollTarget, isAwaitingOilSlickTarget, isAwaitingRayTarget, isSelectingTeleportAlly, isSelectingTeleportShroom, isSelectingSporeBombShroom, playerWhoGotFirstblood, teleportAllyPieceId, addLog, clickedEnemySquare, isSpec, processMoveEnd]);
  ...
  const performAiMove = useCallback(async () => {
    ...
    const gs = adaptBoardForAI(board, 'black', killStreaks, capturedPieces, 0, firstBloodAchieved, playerWhoGotFirstblood, enPassantTargetSquare, lastMovedPieceType, shroomSpawnCounter, nextShroomSpawnTurn, lastMovedPieceHeldItem, lastMovedPieceLevel, didCaptureLastTurn.white, positionHistory, necroResurrectionCounter);
    ...
    if (res?.move) {
        ...
        const appRes = applyMove(board, { from: fromAlg, to: toAlg, type: move.type as Move['type'], grappledFrom: move.grappledFrom ? (Array.isArray(move.grappledFrom) ? coordsToAlgebraic(move.grappledFrom[0], move.grappledFrom[1]) : move.grappledFrom) : undefined }, enPassantTargetSquare, capturedPieces, lastMovedPieceType, lastMovedPieceHeldItem, lastMovedPieceLevel, didCaptureLastTurn.white);
        const totalCaptures = (appRes.capturedPiece ? 1 : 0) + (appRes.pieceCapturedByAnvil ? 1 : 0) + (appRes.selfDestructCaptures?.length || 0);
        const captureGainExp = appRes.capturedPiece ? (DUNGEON_EXP_MAP[appRes.capturedPiece.type] || 1) : 0;
        if (totalCaptures > 0) { audioManager.playCapture(); addEffect('poof', toAlg); }
        if (captureGainExp > 0) addEffect('level-change', toAlg, 'black', captureGainExp);
        ...
        setTimeout(() => { 
          ...
          const oS = killStreaks['black'], nS = (totalCaptures > 0) ? oS + totalCaptures : 0, isExtraTurn = appRes.extraTurn || (oS < 6 && nS >= 6);
        ...
        if (nextNo >= 3) { 
            ...
            // Execute Floor Collapse self-destruct sequence
            let nB = board.map(r => r.map(s => ({...s, piece: s.piece ? {...s.piece} : null, item: s.item ? {...s.item} : null})));
            const nG = { white: Array.isArray(capturedPieces.white) ? [...capturedPieces.white] : [], black: Array.isArray(capturedPieces.black) ? [...capturedPieces.black] : [] };
            
            const blackSquares = nB.flat().filter(sq => sq.piece?.color === 'black');
            blackSquares.forEach(sq => {
                const { rowIndex: r, colIndex: c } = sq;
                addEffect('explosion', sq.algebraic);
                // 3x3 radius explosion
                for (let dr = -1; dr <= 1; dr++) {
                    for (let dc = -1; dc <= 1; dc++) {
                        const nr = r + dr; const nc = c + dc;
                        if (isValidSquare(nr, nc)) {
                            const victimSq = nB[nr][nc];
                            if (victimSq.piece && victimSq.piece.color === 'white' && victimSq.piece.type !== 'king' && victimSq.piece.heldItem !== 'blast_shield') {
                                nG.white.push({ ...victimSq.piece });
                                nB[nr][nc].piece = null;
                            }
                            if (victimSq.item?.type === 'anvil') nB[nr][nc].item = null;
                        }
                    }
                }
                nB[r][c].piece = null;
            ...
            setBoard(nB);
            setCapturedPieces(nG);
            
            const survivors = nB.flat().filter(sq => sq.piece && sq.piece.color === 'white').map(sq => sq.piece!);
            setTimeout(() => advanceLevel(survivors, nG), 1200);
        } else {
            addLog(`The Dungeon is frozen... (${nextNo}/3)`);
            setCurrentPlayer('white'); // Pass turn back to player to allow breaking the stalemate
        }
        setIsAiThinking(false);
    }
  }, [board, currentPlayer, gameInfo.gameOver, isMoveProcessing, isAiThinking, killStreaks, capturedPieces, firstBloodAchieved, playerWhoGotFirstblood, enPassantTargetSquare, lastMovedPieceType, shroomSpawnCounter, nextShroomSpawnTurn, lastMovedPieceHeldItem, lastMovedPieceLevel, didCaptureLastTurn, positionHistory, processPawnSacrificeCheck, addLog, addEffect, advanceLevel, aiNoMoveCounter]);
  ...
  const deskLayout = useMemo(() => (
    <div className="relative z-20 hidden lg:flex flex-row items-start justify-center gap-4 w-full h-full p-4">
      ...
      <div className="w-1/2 flex flex-col items-center gap-2">
        ...
        <div className="w-full"> <ChessBoard boardState={board} selectedSquare={selectedSquare} possibleMoves={possibleMoves} enemySelectedSquare={enemySelectedSquare} enemyPossibleMoves={enemyPossibleMoves} onSquareClick={handleSquareClick} playerColor="white" currentPlayerColor={currentPlayer} isInteractionDisabled={isMoveProcessing || gameInfo.gameOver || isAiThinking || (isSpec && currentPlayer === 'white')} playerInCheck={gameInfo.playerWithKingInCheck} viewMode="flipping" animatedSquareTo={animatedSquareTo} lastMoveFrom={lastMoveFrom} lastMoveTo={lastMoveTo} isAwaitingPawnSacrifice={isAwaitingPawnSacrifice} playerToSacrificePawn={playerToSacrificePawn} isEnPassantTarget={enPassantTargetSquare} onPieceHover={handlePieceHover} effects={effects} promotingSquare={promotionSquare} isAwaitingAnvilDrop={isAwaitingAnvilDrop || isAwaitingAnvilScrollTarget} playerToDropAnvil={playerToDropAnvil} isInventoryOpen={isInventoryOpen} selectedInventoryItemType={selectedInventoryItemType} localPlayerColor="white" isAwaitingHolyShield={isAwaitingHolyShield} isAwaitingArcherSnipe={isAwaitingArcherSnipe} isAwaitingGrappleThrow={isAwaitingGrappleThrow} isAwaitingDanceTarget={isAwaitingDanceTarget} dancerToDance={dancerToDance} grappledPieceSubject={grappledPieceSubject} isAwaitingEarthquakeScrollTarget={isAwaitingEarthquakeScrollTarget} isSelectingMycoSpell={isSelectingMycoSpell} isSelectingTeleportAlly={isSelectingTeleportAlly} isSelectingTeleportShroom={isSelectingTeleportShroom} isSelectingSporeBombShroom={isSelectingSporeBombShroom} isAwaitingCommanderPromotion={isAwaitingCommanderPromotion} playerToPromoteCommander={playerWhoGotFirstblood} isAwaitingWindScrollTarget={isAwaitingWindScrollTarget} isAwaitingShieldScrollTarget={isAwaitingShieldScrollTarget} isAwaitingSwapScrollTarget={isAwaitingSwapScrollTarget} isAwaitingDecreeTarget={isAwaitingDecreeTarget} isAwaitingOilSlickTarget={isAwaitingOilSlickTarget} isAwaitingRayTarget={isAwaitingRayTarget} /> </div>
      </div>
      <div className="w-1/4 flex flex-col gap-4"> <AuthWidget /> <Card className="border-2 border-border/50 bg-card"> <CardContent className="p-4 flex flex-col gap-3"> <Button variant="outline" className="h-12 text-[10px] uppercase gap-2 border-2 text-yellow-500 border-border/50 hover:bg-muted w-full" onClick={() => setIsInventoryOpen(true)} disabled={hasStartedFloor}> <Package className="h-5 w-5 text-yellow-500" /> LOOT BAG </Button> <Button variant="outline" className="h-12 text-[10px] uppercase gap-2 border-2 text-yellow-500 border-border/50 hover:bg-muted w-full" onClick={() => setIsRulesDialogOpen(true)}> <BookOpen className="h-5 h-5 text-yellow-500" /> RULES </Button> <Button variant="outline" className="h-12 text-[10px] uppercase gap-2 border-2 border-border/50 hover:bg-muted w-full" onClick={() => setIsResetConfirmOpen(true)}> <RotateCcw className="h-5 w-5" /> RESET RUN </Button> </CardContent> </Card> </div>
    </div>
  ), [level, statMsg, board, selectedSquare, possibleMoves, enemySelectedSquare, enemyPossibleMoves, handleSquareClick, currentPlayer, isMoveProcessing, gameInfo.gameOver, isAiThinking, isSpec, lastMoveFrom, lastMoveTo, isAwaitingPawnSacrifice, playerToSacrificePawn, enPassantTargetSquare, handlePieceHover, effects, promotionSquare, isAwaitingAnvilDrop, isAwaitingAnvilScrollTarget, playerToDropAnvil, isInventoryOpen, selectedInventoryItemType, isAwaitingHolyShield, isAwaitingArcherSnipe, grappledPieceSubject, isAwaitingEarthquakeScrollTarget, isSelectingMycoSpell, isSelectingTeleportAlly, isSelectingTeleportShroom, isSelectingSporeBombShroom, isAwaitingCommanderPromotion, playerWhoGotFirstblood, isAwaitingWindScrollTarget, isAwaitingShieldScrollTarget, isAwaitingSwapScrollTarget, isAwaitingDecreeTarget, isAwaitingOilSlickTarget, isAwaitingRayTarget, cPanel, hasStartedFloor]);
