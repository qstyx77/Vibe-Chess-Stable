...
function generateDungeonFloor(level: number, playerArmy: Piece[]): BoardState {
  ...
  const frontline = playerArmy.filter(p => ['pawn', 'dancer', 'mimic', 'grappler', 'myco_mage', 'commander', 'infiltrator'].includes(p.type)).sort(() => Math.random() - 0.5);
  ...
  if (isBossLevel) {
    ...
    switch (bossLevelIndex) {
      ...
      case 3: 
        ...
        for(let i=0; i<8; i++) { if (i === 3 || i === 4) continue; board[1][i].piece = { id: `shield-${i}`, type: 'pawn', color: 'black', level: 4, hasMoved: false, isShielded: true, isPoisoned: false, cooldownTurnsRemaining: 0, frozenTurnsRemaining: 0, heldItem: null }; }
...
export default function DungeonPage() {
            ...
            if (finalNRC >= 5) {
                ...
                if (nextGraveyard[myPile].length > 0) {
                    ...
                    if (choice && empty.length > 0) {
                        ...
                        if ((['pawn', 'dancer', 'mimic', 'grappler', 'myco_mage'].includes(res.type)) && row === 7) res.type = 'queen';
  ...
  const triggerSpecialsChain = useCallback((boardToChain: BoardState, currentGraveyard: { white: Piece[], black: Piece[] }, currentKs: { white: number, black: number }, oldStreak: number, newStreak: number, isExtra: boolean, nextEp: AlgebraicSquare | null, actingPlayer: PlayerColor = 'white', completedMilestones: string[] = []) => {
    ...
    if (newStreak >= 1 && oldStreak < 1 && !completedMilestones.includes('dance')) {
        ...
        if (hasDancers) {
            ...
            } else { setSpecialActionContext({ extra: isExtra, nextEp, oldStreak, newStreak, completedMilestones: [...completedMilestones, 'dance'], actingPlayer, currentGraveyard: nextGraveyard, currentKs }); setIsAwaitingDanceTarget(true); addLog("Dancer Skill: The Dance is ready!"); return; }
        }
    }
    if (!firstBloodAchieved && newStreak > 0 && !completedMilestones.includes('firstBlood')) {
        ...
        if (isAI) {
            ...
            addLog("First Blood! Dungeon has promoted a Commander."); triggerSpecialsChain(nextBoard, nextGraveyard, currentKs, oldStreak, newStreak, isExtra, nextEp, actingPlayer, [...completedMilestones, 'firstBlood']); return;
        } else {
    ...
    if (newStreak >= 2 && oldStreak < 2 && !completedMilestones.includes('shield')) {
        ...
        if (hasArchbishop) {
            if (isAI) {
                ...
                addLog("Archbishop Skill: Dungeon shielded an ally!"); triggerSpecialsChain(nextBoard, nextGraveyard, currentKs, oldStreak, newStreak, isExtra, nextEp, actingPlayer, [...completedMilestones, 'shield']); return;
            } else { 
                ...
                if (hasEligibleTargets) { setSpecialActionContext({ extra: isExtra, nextEp, oldStreak, newStreak, completedMilestones: [...completedMilestones, 'shield'], actingPlayer, currentGraveyard: nextGraveyard, currentKs }); setIsAwaitingHolyShield(true); addLog("Holy Shield ready!"); return; } 
    ...
    if (newStreak >= 3 && oldStreak < 3 && !completedMilestones.includes('anvil')) {
        if (isAI) {
            ...
            addLog("Dungeon dropped an Anvil!"); triggerSpecialsChain(nextBoard, nextGraveyard, currentKs, oldStreak, newStreak, isExtra, nextEp, actingPlayer, [...completedMilestones, 'anvil']); return;
        ...
        if (nextGraveyard[myPile].length > 0) {
            ...
            if (choice && empty.length > 0) {
                ...
                if (!isAI && (['pawn', 'dancer', 'mimic', 'grappler', 'myco_mage'].includes(res.type)) && rr === oppBackRank) {
                    setPromotionTargetLevel(1); setPromotionSquare(sq.algebraic); setIsPromotingPawn(true); setSpecialActionContext({ extra: isExtra, nextEp, oldStreak, newStreak, completedMilestones: [...completedMilestones, 'resurrection'], actingPlayer, currentGraveyard: updatedG, currentKs: currentKs }); return;
                }
  ...
  const performAiMove = useCallback(async () => {
    ...
    try {
      ...
      if (aiMove) {
        ...
        if (nextStrikes >= 3) {
            addLog("DUNGEON COLLAPSE! Forces destroyed."); audioManager.playExplosion(); let nextBoard = board.map(r => r.map(s => ({ ...s, piece: s.piece ? { ...s.piece } : null, item: s.item ? { ...s.item } : null })));
            ...
            blackPieceSquares.forEach(({ r, c }) => {
                const alg = coordsToAlgebraic(r, c); addEffect('explosion', alg);
            ...
            if (capturedByCollapse.length > 0) { capturedByCollapse.forEach(p => { const targetPile = p.color; updatedG[targetPile].push(p); }); setCapturedPieces(updatedG); }
  ...
  const handleSquareClick = (algebraic: AlgebraicSquare) => {
    ...
    if (isAwaitingEarthquakeScrollTarget) {
        ...
        const result = applyMove(board, move, enPassantTargetSquare, capturedPieces, lastMovedPieceType, lastMovedPieceHeldItem); const finalizedGraveyard = { ...capturedPieces }; setBoard(result.newBoard); audioManager.playExplosion();
    ...
    if (selectedSquare) {
      ...
      if (isExecutionAllowed && movingPiece && movingPiece.color === currentPlayer) {
        ...
        if (isMoveInFreshList) {
          ...
          if (result.itemReturned) { setInventory(prev => { const next = [...prev]; const existing = next.find(i => i.type === result.itemReturned); if (existing) existing.count++; else next.push({ type: result.itemReturned!, count: 1 }); return next; }); addLog(`Dungeon Item Dropped: ${ITEM_METADATA[result.itemReturned].name}`); }
          if (reflectionOccurred) { const victim = { ...capturedPiece!, id: capturedPiece!.id }; const targetPile = victim.color; updatedGraveyard[targetPile].push(victim); setCapturedPieces(updatedGraveyard); audioManager.playCapture(); addLog("REFLECTED! Dungeon target used Mirror Shield."); addEffect('poof', algebraic); const newKs = { white: 0, black: 0 }; setKillStreaks(newKs); setBoard(newBoard); setTimeout(() => { setSelectedSquare(null); setPossibleMoves([]); setIsMoveProcessing(false); clickGuard.current = false; processMoveEnd(newBoard, updatedGraveyard, newKs, 'white', false, null); }, 800); return; }
          ...
          let resPromoRequired = false; let resResult_promo_level = 1; let resResult_promo_square = null; const landedPiece = newBoard[row][col].piece; const oppBackRank = movingPiece.color === 'white' ? 0 : 7;
          ...
          setTimeout(() => {
            ...
            if (FRONTLINE_TYPES.includes(newBoard[row][col].piece?.type || '') && row === oppBackRank) { queue.push({ square: algebraic, targetLevel: getPromotionLevel(capturedPiece?.type || result.pieceCapturedByAnvil?.type || null) }); }
            if (queue.length > 0) { setPromotionQueue(queue); setPromotionTargetLevel(queue[0].targetLevel); setIsPromotingPawn(true); setPromotionSquare(queue[0].square); setSpecialActionContext({ extra: isExtra, nextEp, oldStreak, newStreak, actingPlayer: 'white', completedMilestones: [], currentGraveyard: updatedGraveyard, currentKs }); addLog("Pawn Promotion ready!"); } 
            else {
