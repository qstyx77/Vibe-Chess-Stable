...
export default function DungeonPage() {
  ...
  const processMoveEnd = useCallback((boardAfter: BoardState, currentGraveyard: { white: Piece[], black: Piece[] }, currentKs: { white: number, black: number }, turnPlayer: PlayerColor, extra: boolean, nextEpSquare: AlgebraicSquare | null = null) => {
    ...
    if (!extra && turnPlayer === 'white' && isKingInCheck(nextBoard, 'white', nextEpSquare, lastMovedPieceType, lastMovedPieceHeldItem)) {
      const msg = "SPLIT SELF-CHECK! Auto-loss.";
    ...
    if (inCheck && extra) {
        if (turnPlayer === 'white') {
            addLog("Boss vanquished in extra turn!");
            advanceLevel(survivors, nextGraveyard);
        } else {
            const msg = "YOUR KING HAS FALLEN! Dungeon Victory.";
    ...
    if (enemyCount === 0 || isDungeonCheckmated) {
      ...
      addLog(isDungeonCheckmated ? "CHECKMATE! Floor Vanquished." : "ALL ENEMIES OBLITERATED! Advancing...");
  ...
  const triggerSpecialsChain = useCallback((boardToChain: BoardState, currentGraveyard: { white: Piece[], black: Piece[] }, currentKs: { white: number, black: number }, oldStreak: number, newStreak: number, isExtra: boolean, nextEp: AlgebraicSquare | null, actingPlayer: PlayerColor = 'white', completedMilestones: string[] = []) => {
            ...
            if (isAI) {
                ...
                if (aiDancer) {
                    ...
                    if (isValidSquare(row+1, col) && !nextBoard[row+1][col].piece && !nextBoard[row+1][col].item) {
                        ...
                        addLog("Dungeon piece performed a Dance move!");
                    }
                }
    ...
    if (!firstBloodAchieved && newStreak > 0 && !completedMilestones.includes('firstBlood')) {
        ...
        if (isAI) {
            ...
            addLog("First Blood! Dungeon has promoted a Commander.");
            ...
            if (hasL1Targets) { setSpecialActionContext({ extra: isExtra, nextEp, oldStreak, newStreak, completedMilestones: [...completedMilestones, 'firstBlood'], actingPlayer, currentGraveyard: nextGraveyard, currentKs }); setIsAwaitingCommanderPromotion(true); addLog("First Blood! Choose a Pawn to promote."); return; }
        }
    }
    if (newStreak >= 2 && oldStreak < 2 && !completedMilestones.includes('shield')) {
        ...
        if (hasArchbishop) {
            if (isAI) {
                ...
                addLog("Archbishop Skill: Dungeon shielded an ally!");
                ...
                if (hasEligibleTargets) {
                    setSpecialActionContext({ extra: isExtra, nextEp, oldStreak, newStreak, completedMilestones: [...completedMilestones, 'shield'], actingPlayer, currentGraveyard: nextGraveyard, currentKs }); setIsAwaitingHolyShield(true); addLog("Holy Shield ready!"); return; 
                } else {
    ...
    if (isSnipeTime) {
        ...
        if (victims.length > 0) {
            if (isAI) {
                ...
                addLog(`Dungeon Sniper destroyed your ${sniped.type}!`);
            ...
            } else { setSpecialActionContext({ extra: isExtra, nextEp, oldStreak, newStreak, completedMilestones: [...completedMilestones, 'snipe'], actingPlayer, currentGraveyard: nextGraveyard, currentKs }); setIsAwaitingArcherSnipe(true); addLog("Sniper Skill: Select a target!"); return; }
        }
    }
    if (newStreak >= 3 && oldStreak < 3 && !completedMilestones.includes('anvil')) {
        if (isAI) {
            ...
            addLog("Dungeon dropped an Anvil!");
        ...
        } else { setSpecialActionContext({ extra: isExtra, nextEp, oldStreak, newStreak, completedMilestones: [...completedMilestones, 'anvil'], actingPlayer, currentGraveyard: nextGraveyard, currentKs }); setIsAwaitingAnvilDrop(true); addLog("Anvil Drop ready!"); return; }
    }
      ...
      if (boardAfter.flat().some(sq => sq.piece && sq.piece.color === player && ['pawn', 'dancer', 'mimic', 'grappler', 'myco_mage', 'commander'].includes(sq.piece.type))) {
        ...
        if (isAI) {
            ...
            if (pawnSq) {
                ...
                audioManager.playCapture();
                addLog(`Dungeon sacrificed ${sacrificed.type} for the Queen!`);
  ...
  const performAiMove = useCallback(async () => {
    ...
    try {
      ...
      if (aiMove) {
        ...
        if (shroomConsumed) { audioManager.playShroom(); addLog("Dungeon piece consumed a Shroom!"); }
        ...
        else if (capturedPiece || (selfDestructCaptures && selfDestructCaptures.length > 0)) {
            audioManager.playCapture(); 
            if (capturedPiece) addLog(`Dungeon captured your ${capturedPiece.type}!`);
        }
        else {
        ...
        setTimeout(() => {
            ...
            if ((['pawn', 'dancer', 'mimic', 'grappler', 'myco_mage'].includes(landedPiece?.type || '')) && (aiMove.to[0] === 7)) { 
              const promoTo = aiMove.promoteTo || 'queen';
              landedPiece!.type = promoTo; 
              ...
              audioManager.playLevelUp(); 
              addLog(`Dungeon promoted to ${promoTo}!`);
            ...
            if (result.multiPromotions && result.multiPromotions.length > 0) {
              result.multiPromotions.forEach(promo => {
                ...
                if (newBoard[pr][pc].piece) {
                  ...
                  addLog("Dungeon multi-promotion!");
                }
              });
            }
  ...
  const handleMycoSpellSelect = useCallback((spell: MycoSpell) => {
      ...
      if (spell === 'propagate') {
        ...
        setTimeout(() => { setIsMoveProcessing(false); clickGuard.current = false; setSelectedSquare(null); processMoveEnd(applyResult.newBoard, capturedPieces, killStreaks, currentPlayer, false, null); }, 800);
          ...
          addLog("Mushroomancy: Teleport! Select an allied piece.");
      } else if (spell === 'spore-bomb') {
          setIsSelectingSporeBombShroom(true);
          addLog("Mushroomancy: Spore Bomb! Select a shroom to detonate.");
  ...
  const handleSquareClick = (algebraic: AlgebraicSquare) => {
    ...
    if (isSelectingTeleportAlly) {
        if (piece && piece.color === currentPlayer && piece.type !== 'king' && piece.type !== 'queen' && piece.id !== board[algebraicToCoords(selectedSquare!).row][algebraicToCoords(selectedSquare!).col].piece?.id) {
            ...
            addLog(`Teleporting ${piece.type}! Select a destination shroom.`);
        }
        return;
    }
    ...
    if (isAwaitingDanceTarget) {
        ...
        if (isOneForward || isAdjacent) {
            ...
            if (isOneForward && !nextBoard[row][col].piece && !nextBoard[row][col].item) {
                ...
                addLog(`Dancer Skill: Captured ${captured.type}!`);
            } else { return; }
    ...
    if (selectedSquare) {
      ...
      if (isExecutionAllowed && movingPiece && movingPiece.color === currentPlayer) {
        ...
        if (isMoveInFreshList) {
          ...
          else if (streakGain > 0) {
              audioManager.playCapture();
              if (capturedPiece) addLog(`Captured ${capturedPiece.type} at ${algebraic}!`);
          }
          else {
              audioManager.playMove();
              addLog(`${movingPiece.type} to ${algebraic}`);
          }
    ...
    <div className="relative z-10 flex flex-col flex-grow w-full max-w-lg mx-auto p-2">
      <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex items-center gap-2">
             ...
             <span className="font-pixel text-[12px] uppercase">Floor {level}</span>
          </div>
      ...
      <ChessBoard 
        boardState={board} 
        selectedSquare={isAnySpecialModeActive ? (isAwaitingDanceTarget ? dancerToDance : (isAwaitingGrappleThrow ? selectedSquare : null)) : selectedSquare} 
        possibleMoves={isAnySpecialModeActive ? [] : possibleMoves} 
        onSquareClick={handleSquareClick} 
        playerColor="white" 
        currentPlayerColor={currentPlayer} 
        isInteractionDisabled={isMoveProcessing || gameInfo.gameOver || (isAnySpecialModeActive && currentPlayer === 'white')} 
        ...
        isAwaitingGrappleThrow={isAwaitingGrappleThrow}
        isAwaitingDanceTarget={isAwaitingDanceTarget}
        dancerToDance={dancerToDance}
        grappledPieceSubject={grappledPieceSubject}
        isAwaitingEarthquakeScrollTarget={isAwaitingEarthquakeScrollTarget}
        isSelectingMycoSpell={isSelectingMycoSpell}
        isSelectingTeleportAlly={isSelectingTeleportAlly}
        isSelectingTeleportShroom={isSelectingTeleportShroom}
        isSelectingSporeBombShroom={isSelectingSporeBombShroom}
        isAwaitingCommanderPromotion={isAwaitingCommanderPromotion}
        playerToPromoteCommander={currentPlayer === 'white' ? 'white' : null}
    ...
    <div className="relative z-10 flex flex-row items-start justify-center gap-6 w-full h-full p-6 max-w-7xl mx-auto">
      ...
      <div className="w-1/2 flex flex-col items-center gap-4">
        <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl font-pixel text-primary uppercase tracking-tighter shadow-sm">Dungeon Floor {level}</h1>
            <Skull className="h-8 w-8 text-destructive animate-pulse" />
        </div>
        
        <div className="text-center font-pixel text-sm text-foreground/80 min-h-[1.5em] uppercase">
            {isInventoryOpen ? "SELECT AN ITEM TO EQUIP!" : isAwaitingPawnSacrifice ? "ROYAL SACRIFICE REQUIRED!" : isPromotingPawn ? "PROMOTE YOUR PAWN!" : isAiThinking ? "DUNGEON IS THINKING..." : gameInfo.message}
        </div>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 flex-grow flex flex-col h-full overflow-y-auto">
        <div className="lg:hidden flex-grow">{mobileLayout}</div>
        <div className="hidden lg:flex flex-grow">{desktopLayout}</div>
      </div>

      <InventoryWindow isOpen={isInventoryOpen} onClose={() => setIsInventoryOpen(false)} inventory={inventory} selectedItemType={selectedInventoryItemType} onSelectItem={setSelectedInventoryItemType} onUseItem={(type) => { if (type.startsWith('portal_scroll_')) { warpToLevel(parseInt(type.split('_')[2]), type); } }} usedSlots={usedSlots} attunementSlots={attunementSlots} />
      ...
      <AlertDialog open={isResetConfirmOpen} onOpenChange={setIsResetConfirmOpen}>
        <AlertDialogContent className="font-pixel border-2 border-destructive/50">
          <AlertDialogHeader>
            <AlertDialogTitle className="uppercase text-destructive">Abandon Current Run?</AlertDialogTitle>
            <AlertDialogDescription className="text-[10px] uppercase text-muted-foreground">This will wipe your progress and equipment, sending you back to Floor 1.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="text-[10px] uppercase h-9">Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleResetRun} className="bg-destructive text-[10px] uppercase h-9">Abandon Run</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
