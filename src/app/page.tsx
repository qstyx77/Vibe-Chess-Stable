      ...
      if (boardAfter.flat().some(sq => sq.piece && sq.piece.color === player && ['pawn', 'dancer', 'mimic', 'grappler', 'commander'].includes(sq.piece.type))) {
        ...
        if (isAI) {
            ...
            if (pawnSq) {
                ...
                triggerSpecialsChain(nextB, graveyard, currentKs, oldS, newS, extra, ep, player, []);
            }
            return true;
        }
    ...
    if (isAwaitingDanceTarget) {
        ...
        if (isOneForward || isAdjacent) {
            ...
            if (isOneForward && !nextBoard[row][col].piece && !nextBoard[row][col].item) {
                ...
                nextG[currentPlayer] = [...(nextG[currentPlayer] || []), { ...victim, id: `dance_${Date.now()}` }];
            } else { return; }
    ...
    if (isAwaitingPawnSacrifice) {
      if (piece && ['pawn', 'dancer', 'mimic', 'grappler', 'commander'].includes(piece.type) && piece.color === currentPlayer) {
        if (onlineStatus === 'connected') {
            ...
            nextG[currentPlayer === 'white' ? 'black' : 'white'] = [...(nextG[currentPlayer === 'white' ? 'black' : 'white'] || []), sacrificed];
        ...
        if (piece && piece.color === oppColor && piece.type !== 'king' && piece.type !== 'queen') {
            ...
            if (responsibleArcher) {
                if (onlineStatus === 'connected') {
                    ...
                    nextG[currentPlayer] = [...(nextG[currentPlayer] || []), snipedPiece];
