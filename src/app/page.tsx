...
export default function EvolvingChessPage() {
  ...
  const handleSquareClick = useCallback((algebraic: AlgebraicSquare) => {
    ...
    if (isAwaitingPawnSacrifice && isLocalActionTurn) {
      if (clickedPiece && (clickedPiece.type === 'pawn' || clickedPiece.type === 'commander') && clickedPiece.color === currentPlayer) {
        ...
        triggerSpecialsChain(boardAfterSacrifice, anvilDropContext?.oldStreak || 0, anvilDropContext?.newStreak || 0, isExtraTurnFromQueenMove, anvilDropContext?.newEnPassantTarget || null, currentPlayer, anvilDropContext?.completedMilestones || []);
      }
      return;
    }
    ...
    if (isAwaitingCommanderPromotion && playerWhoGotFirstBlood === currentPlayer) {
        if (clickedPiece && clickedPiece.type === 'pawn' && clickedPiece.color === currentPlayer && clickedPiece.level === 1) {
            ...
            triggerSpecialsChain(boardAfterCmdr, anvilDropContext?.oldStreak || 0, anvilDropContext?.newStreak || 0, anvilDropContext?.isExtraTurn || false, anvilDropContext?.newEnPassantTarget || null, currentPlayer, anvilDropContext?.completedMilestones || []);
        }
        return;
    }
    ...
    if (selectedSquare) {
      ...
      if (freshlyCalculatedMoves.includes(algebraic)) {
        ...
        setTimeout(() => {
          ...
          if (pieceAtDest?.type === 'queen' && originalT === 'queen') {
              handled = processPawnSacrificeCheck(nextBoard, currentPlayer, {from: selectedSquare, to: algebraic, type: 'move'}, originalL, originalT, isExtra, nextEp, oldStreak, newStreak);
          }
          ...
          if (pieceAtDest?.type === 'pawn' && (row === 0 || row === 7)) {
              ...
              setPromotionTargetLevel(getPromotionLevel(result.capturedPiece?.type || result.pieceCapturedByAnvil?.type || null)); 
              setIsPromotingPawn(true); 
              setPromotionSquare(algebraic); 
              setAnvilDropContext({ boardForNextStep: nextBoard, playerWhoseTurnCompleted: currentPlayer, isExtraTurn: isExtra, newEnPassantTarget: nextEp, oldStreak, newStreak, completedMilestones: [] });
              handled = true;
          }
          
          if (!handled) {
            triggerSpecialsChain(nextBoard, oldStreak, newStreak, isExtra, nextEp, currentPlayer, []);
          }
        }, 800);
        return;
      }
  ...
  }, [board, promotionSquare, promotionTargetLevel, anvilDropContext, triggerSpecialsChain, onlineStatus, currentPlayer]);
  ...
  return (
    <div className={cn("min-h-full h-full w-full bg-background flex flex-col relative after:content-[''] after:fixed after:inset-0 after:bg-black after:opacity-0 after:-z-10 after:pointer-events-none", showLossScreen && "after:animate-fade-to-black")}>
      ...
      <InventoryWindow
        ...
        onUseItem={(type) => {
            // Lobby usage - Currently portals are only for Dungeon, but we prevent equipping here
        }}
