        ...
        <div className="space-y-4 py-4">
          <div className="grid grid-cols-2 gap-4 text-center">
            <div className="p-3 bg-muted/30 border border-border rounded-none">
              <p className="text-[10px] text-muted-foreground uppercase font-bold">Total Moves</p>
          ...
          {eloInfo && (
            <div className="p-3 bg-primary/5 border border-primary/10 space-y-2">
              <h4 className="text-[10px] font-bold text-muted-foreground uppercase">Ranked Summary (ELO)</h4>
              <div className="space-y-2">
                {Object.entries(eloInfo).map(([uid, info]: [string, any]) => (
                  <div key={uid} className="flex justify-between items-center text-sm">
                    <span className="font-medium text-xs opacity-70">Player</span>
                    <div className="flex items-center gap-2">
                      <span className="text-muted-foreground line-through decoration-destructive/50">{info.oldElo}</span>
                      <span className="text-primary font-bold">→ {info.newElo}</span>
