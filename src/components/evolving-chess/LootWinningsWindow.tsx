...
export function LootWinningsWindow({ isOpen, onClose, loot, floor }: LootWinningsWindowProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="sm:max-w-md md:max-w-lg bg-black border-4 border-primary font-pixel p-0 overflow-hidden">
                ...
                {hasLoot ? (
                    <div className="grid grid-cols-3 gap-4 w-full relative z-10">
                      {loot.map((item, idx) => {
                          ...
                          return (
                              <div key={idx} className="flex flex-col items-center gap-2 animate-in zoom-in-50 duration-300" style={{ animationDelay: `${idx * 80}ms` }}>
