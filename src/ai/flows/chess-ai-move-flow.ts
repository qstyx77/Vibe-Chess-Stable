...
**CRITICAL SAFETY INSTRUCTION: IF YOUR ({{{playerColor}}}) KING IS CURRENTLY IN CHECK, your *absolute highest priority* is to make a move that gets your King out of check. This can be achieved by: 1. Moving your King to a safe square. 2. Blocking the check with another piece. 3. CAPTURING THE PIECE THAT IS DELIVERING THE CHECK (this is often the best option if available and safe). You MUST find a legal move that resolves the check. All other strategic considerations are secondary until your King is safe. If no such move exists, it is checkmate, but you should still try to output a valid "from" and "to" if you believe you have a last resort move, or explain if you believe it's checkmate in the reasoning.**

Your goal is to choose the best possible move.
**ABSOLUTE HIGHEST PRIORITY: KING SAFETY.** First, ensure your King is not in check, or if it is, that your move resolves the check. After King safety is assured, consider the following strategic elements in roughly this order of importance:

1.  **AGGRESSIVE CAPTURES & MATERIAL ADVANTAGE (CRUCIAL PRIORITY):** Your primary offensive goal is to gain a material advantage by capturing enemy pieces.
    *   **Prioritize Legal Captures:** Actively seek out and execute legal captures. If a legal capture is available, especially of a higher-value piece (Queen > Rook > Bishop/Knight > Pawn) or a piece posing an immediate threat, **YOU SHOULD ALMOST ALWAYS CHOOSE THIS CAPTURE over a simple pawn move or minor piece repositioning, unless making the capture leads to an immediate checkmate against you or a definite, catastrophic loss of material for no significant gain.** Do not be overly passive.
    *   **Verification is Key:** As always, meticulously verify the legality of the capture: the piece can make the move, the path is clear if required, the target is not invulnerable to your piece (e.g., high-level Queen vs. lower-level attacker, invulnerable Rook, Bishop vs. Pawn), and the move does not put your own King in check.
