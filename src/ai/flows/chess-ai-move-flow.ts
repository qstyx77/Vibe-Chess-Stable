...
4.  **CREATE THREATS, CONTROL SPACE, & IMPROVE POSITION (Work Towards Checkmate):**
    ...
    *   **Dominate Key Squares & Center Control:** Control central squares (d4, e4, d5, e5).
    *   **Piece Development & Activity (Mobility):** Especially in the early game (first ~5-10 moves), move your Knights and Bishops off their starting squares towards the center or influential positions where they control more squares and have more options. For instance, after moving a center pawn, developing a Knight to support it (e.g., to c3/f3 for White or c6/f6 for Black) is often a good follow-up. Do not just move pawns if no better capturing, threatening, or developing moves are available. Aim for active piece placement where your pieces control many squares, have good mobility, and coordinate well. Consider outposts for knights and open files/diagonals for rooks and bishops.
...
If it is your first move of the game, aim for a solid opening. Standard chess openings often focus on controlling the center (e.g., by moving a center pawn like e2-e4 or d2-d4 if white, or responding with e7-e5 or d7-d5 if black) and rapidly developing your Knights and Bishops to active squares (e.g., Knights to c3, f3, c6, f6; Bishops to c4, f4, c5, f5). For instance, after 1.e4 e5, developing a Knight with 2.Nf3 is very common. Prioritize King safety and piece development in these initial moves. After your first move, continue to prioritize piece development and central control for the next few moves. BEFORE outputting this move, you MUST simulate it in your mind: confirm the piece exists, is yours, and that the path and destination square are valid according to all rules for that piece type and its current level.
