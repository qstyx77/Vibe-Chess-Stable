...
{{else}}
{{#if isPlayerInCheck}}
**YOUR KING IS IN CHECK. You MUST make a move that resolves the check. Prioritize moves that directly move your King to safety or capture the attacker if possible.**
{{/if}}
{{/if}}
...
4.  **CREATE THREATS, CONTROL SPACE, & IMPROVE POSITION (Work Towards Checkmate):**
    ...
    *   **Dominate Key Squares & Center Control:** Control central squares (d4, e4, d5, e5) and squares around the opponent's King. Aim for active piece placement where your pieces control many squares, have good mobility, and coordinate well. Consider outposts for knights and open files/diagonals for rooks and bishops.
...
B. For EACH of your pieces {{#if availablePieceSquares}}(especially those on squares: {{#each availablePieceSquares}}{{{this}}}{{#unless @last}}, {{/unless}}{{/each}}){{/if}}, determine ALL its legal moves based on standard chess rules AND all VIBE CHESS abilities for its level. A move is legal if:
    ...
    iv. Crucially, the move does not place or leave your own King in check. If your King starts the turn in check ({{#if isPlayerInCheck}}which it does{{/if}}), this move MUST result in your King no longer being in check (e.g., by **moving the King to a safe square**, blocking the attack, or **CAPTURING THE ATTACKING PIECE - remember the strong advice in the CRITICAL SAFETY INSTRUCTION if the attacker is adjacent to your King**).
